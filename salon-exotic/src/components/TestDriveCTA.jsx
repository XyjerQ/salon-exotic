...
export default function TestDriveCTA() {
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.name.trim() || !form.phone.trim() || !form.date) {
      setStatus({ type: 'error', msg: 'Please fill in your name, phone, and preferred date.' })
      return
    }
    ...
    try {
      const response = await fetch(`${API_BASE}/test-drives`, {
        ...
        body: JSON.stringify({
          ...
          email: form.email,
          date: form.date,
          car_id: null // Brak auta w momencie rezerwacji przez klienta
        }),
      })

      if (response.ok) {
        setStatus({ type: 'success', msg: 'Request saved. Our concierge will call you shortly to confirm.' })
        setForm({ name: '', phone: '', email: '', date: '' })
      } else {
        const errorData = await response.json().catch(() => ({}))
        setStatus({ type: 'error', msg: errorData.error || 'Failed to submit request. Please try again.' })
      }
    } catch (err) {
  ...
  return (
    <section className="text-black py-10 md:py-14" id="contact-form">
      <article ref={sectionRef} className="opacity-0-init max-w-7xl mx-auto grid md:grid-cols-[1fr,1.2fr] items-stretch rounded-xl overflow-hidden bg-black border border-gray-200 shadow-xl">
        <div className="p-8 md:p-10 flex flex-col justify-center gap-4 md:order-1 text-white">
          ...
          <p className="text-gray-300 mt-2">Leave your contact and preferred date. Our concierge will call you to arrange the details and prepare the car.</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4 mt-4">
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1" htmlFor="email">Email (optional)</label>
              <input
                id="email"
                name="email"
                type="email"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blackline-accent text-black font-semibold py-3 rounded-md hover:opacity-90 transition disabled:opacity-50"
            >
